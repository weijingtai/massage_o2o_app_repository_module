...
class OnlineUsersRepository{
  ...
  Future<List<OnlineMasterUserModel>> loadOnlineMastersByUidList(List<SubscribeUserModel> friendsList) async{
    Map<String,SubscribeUserModel> uidSubMapper = <String,SubscribeUserModel>{};
    friendsList.forEach((friend) {uidSubMapper[friend.uid] = friend;});

    logger.i("fetchOnlineMaster by uid list.");
    var remoteOnlineMasterUserModelList = await fetchOnlineMaster(uidSubMapper.keys.toList());
  ...
  Future<List<OnlineMasterUserModel>> fetchOnlineMaster(List<String> uids) async{
    logger.d("load total ${uids.length} friends' uid.");
    var queryTimes = uids.length ~/10;
    if (uids.length % 10 != 0){
      queryTimes ++;
    }
    logger.d("load total ${uids.length} friends' uid. query with $queryTimes times.");
    ...
    if (queryTimes == 1){
      var futureQueryList = await FirebaseFirestore.instance
          ...
          .get();
      logger.v("collectionName $ONLINE_MASTERS_COLLECTION_NAME,futureQueryList ${futureQueryList.size}");
      allOnlineFriendsSnapshotList = futureQueryList.docs
    ...
    logger.v("allOnlineFriendsSnapshotList: ${allOnlineFriendsSnapshotList.length}");
    return allOnlineFriendsSnapshotList.map((snapshot)=>OnlineMasterUserModel.fromJson(snapshot.data()!)).toList();
  }
}