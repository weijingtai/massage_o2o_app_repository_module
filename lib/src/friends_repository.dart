...
class FriendsRepository{
  ...
  Future<List<SubscribeUserModel>> loadByLoggedInUserUid(String loggedInUid,{bool withHostUser = false}) async{
    var roleFilter = ["MASTER"];
    if (withHostUser){
      roleFilter.add("HOST");
    }
    var allResult = await Future.wait([
      FirebaseFirestore.instance.collection("Subscribe")
          .doc(loggedInUid)
    ...
    if (followedListSnapshot.size > 0){
      ...
      followedSet = followedListSnapshot.docs
          .where((e) => e.exists)
          .map((e) => SubscribeUserModel.fromJson(e.data()["baseInfo"])..subscribeStateEnum = SubscribeStateEnum.IS_FOLLOWED)
          .toSet();
    }else{
      logger.d("user's followedSet is empty.");
    }
    if (followerListSnapshot.size >0){
      followerSet = followerListSnapshot.docs
          .where((e) => e.exists)
          .map((e){
        logger.v("${e.data()}");
        return SubscribeUserModel.fromJson(e.data()["baseInfo"])..subscribeStateEnum = SubscribeStateEnum.IS_FOLLOWER;
      })
          .toSet();
    }else{
    ...
    if (followerSet.isNotEmpty && followedSet.isNotEmpty){
      logger.d("both followerSet and followerSet is not empty. handle BI_FOLLOW.");
  ...
  Future<List<String>> loadAllFriendsUid(String loggedInUid,{bool withHostUser = false}) async{
    var roleFilter = ["master"];
    if (withHostUser){
      roleFilter.add("host");
    }
    logger.i("load user's all friends' uid.");
    var allResult = await Future.wait([
      FirebaseFirestore.instance.collection("Subscribe")
          .doc(loggedInUid)
          .collection("FollowedList")
    ...
    if (followedListSnapshot.size > 0 ){
      logger.i("load ${followedListSnapshot.size} followed friends uid.");
      friendsUidList.addAll(followedListSnapshot.docs
          .where((e) => e.exists)
          .where((e){
          var role = e.data()['baseInfo']['role'];
          if (withHostUser){
    ...
    if (followerListSnapshot.size>0){
      logger.i("load ${followerListSnapshot.size} follower friends uid.");
      friendsUidList.addAll(followerListSnapshot.docs
          .where((e) => e.exists)
          .where((e){
            var role = e.data()['baseInfo']['role'];
            if (withHostUser){
    ...
    logger.i("load total ${friendsUidList.length} friends uid.");
    return friendsUidList.toList();
  }
  Set<SubscribeUserModel> getIntersectionSet(Set<SubscribeUserModel> first,Set<SubscribeUserModel> second){
    return first.intersection(second);
  }
  Set<SubscribeUserModel> getDifferenceSet(Set<SubscribeUserModel> first,Set<SubscribeUserModel> second){
