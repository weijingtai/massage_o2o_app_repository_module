...
class AssignRepository {
  ...
  CollectionReference get assignCollection =>
  ...
  AssignRepository({required this.firestore, required this.repositoryConfig})
  ...
  Future<List<AssignModel>?> getAll(List<String> guids) async {
    ...
    List<AssignModel> resultList = queryDocs
        .where((e) => e.exists)
        .map((e) => e.data()! as AssignModel)
        .toList();
  ...
  Future<List<QueryDocumentSnapshot<Object?>>> _loadByGuids(
    ...
    if (guids.length > repositoryConfig.whereInLimit) {
      ...
      Iterable<Future<QuerySnapshot>> getRemoteAssignListFutureList =
          batchGuids.map((assignGuidListChunk) => assignCollection
              .where(config.assignGuidFieldName, whereIn: assignGuidListChunk)
              .get());
      ...
      // get all reference from remote
      var queryResult = await assignCollection
          .where(config.assignGuidFieldName, whereIn: guids)
          .get();
  ...
  Future<void> updateFields(
      String assignGuid, Map<String, dynamic> fieldsMap) async {
    ...
    if (fieldsMap.isNotEmpty) {
      ...
      if (fieldsMap.containsValue(null)) {
        fieldsMap =
            fieldsMap.map((k, v) => MapEntry(k, v ?? FieldValue.delete()));
      }
      logger.v(fieldsMap);
      logger.d("try get old assign from remote.");
      ...
      if (!fieldsMap.containsKey(config.lastModifiedAtFieldName)) {
        fieldsMap[config.lastModifiedAtFieldName] =
  ...
  Future<List<String>?> updateAssignListFields(
      List<String> assignGuidList, Map<String, dynamic> fieldsMap) async {
    ...
    if (assignGuidList.isNotEmpty && fieldsMap.isNotEmpty) {
      ...
      logger.v(fieldsMap);
      List<QueryDocumentSnapshot> allAssignData =
          await _loadByGuids(assignGuidList);
      logger.d(
          "total:${assignGuidList.length} to update, found total:${allAssignData.length} in remote.");
      ...
      if (allAssignData.isNotEmpty) {
        ...
        var allAssignGuid = allAssignData
            .where((e) => e.exists)
            .map((e) => (e.data()! as AssignModel).guid)
            .toList();
        // 2. get not found assignGuidList
        var notFoundAssignGuidList =
            assignGuidList.where((e) => !allAssignGuid.contains(e)).toList();
        if (notFoundAssignGuidList.isNotEmpty) {
          ...
          logger
              .i("updateAssignListFields total:${assignGuidList.length} done.");
          return Future.value([]);
        }
      } else {
  ...
  Future<List<AssignModel>?> deleteAll(List<String> guids) async {
    ...
    if (guids.isNotEmpty) {
      ...
      logger.d(
          "total:${guids.length} to delete, found total:${allAssignData.length} in remote.");
  ...
  Query _queryWithStartEndLimit(
      Query query, DateTime? start, DateTime? end, int? limit) {
    if (start != null) {
      // query = query.startAfter([Timestamp.fromMillisecondsSinceEpoch(start.millisecondsSinceEpoch)]);
      query = query.startAfter([start.toUtc().toIso8601String()]);
    ...
    if (end != null) {
      ...
      query = query.endBefore([end.toUtc().toIso8601String()]);
  ...
  Future<List<AssignModel>> listAllByMasterUid(String masterUid,
      {DateTime? start, DateTime? end, int? limit}) async {
    logger.i(
        "listAllMasterUid masterUid:$masterUid, with start:$start~end:$end,limit:$limit");
    Query query = assignCollection
        .where(config.masterUidFieldName, isEqualTo: masterUid)
        .orderBy(config.createdAtFieldName);
    var snapshot =
        await _queryWithStartEndLimit(query, start, end, limit).get();
    if (snapshot.docs.isNotEmpty) {
      ...
      logger.i(
          "listAllMasterUid masterUid:$masterUid, with start:$start~end:$end,limit:$limit done.");
      logger.v(result);
      return result;
    } else {
      logger.w(
          "not found any assign with masterUid:$masterUid, start:$start~end:$end");
      return Future.value([]);
    }
  }
  ...
  Future<List<AssignModel>> listAllActivatedByMasterUid(
      String masterUid) async {
    ...
    var querySnapshot = await assignCollection
        ...
        .get();
    logger.i(
        "listAllAssigningByMasterUid success with total:${querySnapshot.size}, in assigning state.");
  ...
  /// [start] is the datetime after which assign should be listed
  /// [end] is the max number of assign to be listed
  Future<List<AssignModel>> listAllByHostUid(String hostUid,
      {DateTime? start, DateTime? end, int? limit}) async {
    Query query = assignCollection
    ...
    logger.i(
        "listAllByHostUid hostUid:$hostUid, with start:$start~end:$end,limit:$limit, found:${result.length}");
    logger.v(result);
    return result;
  }
  ...
  Future<List<AssignModel>> listAllByOrderGuidList(
    ...
    if (orderGuidList.length <= repositoryConfig.whereInLimit) {
      ...
      Iterable<List<String>> orderGuidChunks =
      ...
      Iterable<Future<List<AssignModel>>> futureList =
          orderGuidChunks.map((chunk) async {
        var snapshot = await assignCollection
