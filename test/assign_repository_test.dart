...
Future<void> main() async {
  ...
  group("test getAll", () {
    test("test getAll", () async {
      ...
      var result = await testAssignRepository
          .getAll(assignModelList.map((e) => e.guid).toList());
  ...
  group("test add", () {
    test('test add', () async {
      ...
      fakeFirestore
          ...
          .get()
          .then((value) {
        expect(value.data(), newAssignModel.toJson(),
            reason: "saved assignModel should be equal to newAssignModel");
      });
    });
    test("test addAll", () async {
      // await fakeFirestore.clearPersistence();
      List<AssignModel> newAssignModels =
          List<AssignModel>.generate(2, (index) => generateAssignModel());
      await testAssignRepository.addAll(newAssignModels);
      var newQuerySnap =
          await fakeFirestore.collection(ASSIGN_COLLECTION_NAME).get();
      expect(newQuerySnap.size, 6,
          reason:
              "querySnap.size should be 6, after AssignRepository#addAll(newAssignModels)");
    });
  });
  ...
  group("test update", () {
    test("test update", () async {
      ...
      expect(querySnapshot.exists, true,
          reason: "querySnapshot.exists should exits in firebase collection.");
      expect(querySnapshot.data(), oldAssignModel.toJson(),
          reason:
              "querySnapshot.data() should be equal to oldAssignModel.toJson()");
      ...
      var newQuerySnapshot = await fakeFirestore
          ...
          .doc(oldAssignModel.guid)
          .get();
      expect(newQuerySnapshot.exists, true,
          reason:
              "newQuerySnapshot.exists should exits in firebase collection.");
      expect(newQuerySnapshot.data(), newAssignModel.toJson(),
          reason:
              "newQuerySnapshot.data() should be equal to newAssignModel.toJson()");
    });
    test("test update with non-exits doc", () async {
      ...
      var newQuerySnapshot = await fakeFirestore
          ...
          .doc(oldAssignModel.guid)
          .get();
      expect(newQuerySnapshot.exists, false,
          reason:
              "newQuerySnapshot.exists should exits in firebase collection.");
    });
  });
  ...
  group("test updateFields", () {
    test("test updateFields", () async {
      ...
      expect(newQuerySnapshot.data(), newAssignModel.toJson(),
          reason:
              "newQuerySnapshot.data() should be equal to oldAssignModel.toJson()");
    });
  });
  ...
  group("test updateAssignListFields", () {
    test("test updateAssignListFields with 2", () async {
      ...
      await testAssignRepository
          .updateAssignListFields(oldAssignList.map((e) => e.guid).toList(), {
        "deliveredAt": deliveredAt.toUtc().toIso8601String(),
        "state": AssignStateEnum.Assigning.name,
      });

      var newData = await fakeFirestore
          .collection(ASSIGN_COLLECTION_NAME)
          .doc(oldAssignList.first.guid)
          .get();
      expect(newData.exists, true);
      expect(newData["deliveredAt"], deliveredAt.toUtc().toIso8601String());
      expect(newData["state"], AssignStateEnum.Assigning.name);

      newData = await fakeFirestore
          .collection(ASSIGN_COLLECTION_NAME)
          .doc(oldAssignList.last.guid)
          .get();
      expect(newData.exists, true);
      expect(newData["deliveredAt"], deliveredAt.toUtc().toIso8601String());
      expect(newData["state"], AssignStateEnum.Assigning.name);
    ...
    test("test updateAssignListFields with 10", () async {
      ...
      var deliveredAt = DateTime.now();
      var deliveredAtString = deliveredAt.toUtc().toIso8601String();
      await testAssignRepository
          .updateAssignListFields(oldAssignList.map((e) => e.guid).toList(), {
      ...
      await fakeFirestore
          .collection(ASSIGN_COLLECTION_NAME)
          .where("guid", whereIn: oldAssignList.map((e) => e.guid).toList())
          .get()
          .then((value) {
        expect(value.docs.length, oldAssignList.length,
            reason:
                "value.docs.length should be equal to oldAssignList.length");
        for (var i = 0; i < value.docs.length; i++) {
          var doc = value.docs[i];
          expect(doc.data()["deliveredAt"], deliveredAtString,
              reason: "doc.data() should be equal to assignModel.toJson()");
          expect(doc.data()["state"], AssignStateEnum.Assigning.name,
              reason: "doc.data() should be equal to assignModel.toJson()");
        }
      });
    });
    ...
    test("test updateAssignListFields contains not exists", () async {
      ...
      var deliveredAt = DateTime.now();
      List<String>? notFoundAssignGuid = await testAssignRepository
          .updateAssignListFields(updateAssignGuidList, {
        "deliveredAt": deliveredAt.toUtc().toIso8601String(),
        "state": AssignStateEnum.Assigning.name,
      });
  ...
  group("test delete", () {
    test("normal", () async {
      ...
      var deletedAssign =
          await testAssignRepository.delete(newAssignModel.guid);
  ...
  group("test deleteAll", () {
    test("normal", () async {
      var newAssignModelList =
          List.generate(2, (index) => generateAssignModel());
      await testAssignRepository.addAll(newAssignModelList);
      var querySnap = await fakeFirestore
          .collection(ASSIGN_COLLECTION_NAME)
          .where("guid",
              whereIn: newAssignModelList.map((e) => e.guid).toList())
          .get();
      expect(querySnap.size, 2);
      await testAssignRepository
          .deleteAll(newAssignModelList.map((e) => e.guid).toList());
    });
    test("1 not found", () async {
      var newAssignModelList =
          List.generate(2, (index) => generateAssignModel());
      var notExistAssignGuid = Uuid().v4();
      var exitsAssignGuidList = newAssignModelList.map((e) => e.guid).toList()
        ..add(notExistAssignGuid);
      await testAssignRepository.addAll(newAssignModelList);
      var querySnap = await fakeFirestore
          .collection(ASSIGN_COLLECTION_NAME)
          ...
          .get();
      expect(querySnap.size, 2);
      List<AssignModel>? deletedAssignList = await testAssignRepository
          .deleteAll(newAssignModelList.map((e) => e.guid).toList());
      var newQuerySnap = await fakeFirestore
          .collection(ASSIGN_COLLECTION_NAME)
          .where("guid", whereIn: exitsAssignGuidList)
  ...
  group("test listAllByMasterUid", () {
    test("normal without param after", () async {
      ...
      newAssignModelList.first.createdAt =
    ...
    test("normal with param after as now", () async {
      ...
      await testAssignRepository.addAll(newAssignModelList);
      var firstAssignInDB = await fakeFirestore
          .collection(ASSIGN_COLLECTION_NAME)
          .doc(newAssignModelList.first.guid)
          .get();
      var result = await testAssignRepository.listAllByMasterUid(masterUid,
          start: now.subtract(Duration(seconds: 5)));
